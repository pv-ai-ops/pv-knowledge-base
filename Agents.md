...
3. 处理完成后脚本会运行 `hexo clean && hexo generate`，同时启动 `hexo server` 在 `http://localhost:4000/pv-knowledge-base/` 提供预览。
4. 原始素材在成功搬运后会从 `content-inbox/` 中移除，请提前自行备份需要长期保留的源文件。

## 4. 常用命令
- `npm run publish`：执行全量处理 + 构建 + 本地预览（必须完成此步骤以验证结果）。
- `npm run preview`：清理、构建并启动 Hexo 预览服务器；适用于无需重复搬运内容时的检查。
